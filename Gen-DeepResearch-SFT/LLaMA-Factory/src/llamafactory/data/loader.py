...
def _get_preprocessed_dataset(
    ...
    def _safe_batch_wrapper(fn, _log_path=log_path):
        ...
        def _wrap(batch):
            ...
            for i in range(n):
                ...
                single_batched = {k: [v[i]] for k, v in batch.items()}
                try:
                    processed = fn(single_batched)
                    if not processed:
                        continue
            ...
            # The entire batch failed → return empty dict so datasets skips this batch
            return {} if kept == 0 else out_batch
