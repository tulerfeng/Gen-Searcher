        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # 可能出现整批都被过滤掉（全无效/全被截断）的情况：直接跳过本轮更新，避免后续 reshape/平衡分桶等逻辑报错
                    if "attention_mask" not in batch.batch or batch.batch["attention_mask"].shape[0] == 0:
                        solve_none += 1
                        pprint(
                            f"[Skip] empty batch after filtering at step={self.global_steps} (all invalid/truncated)."
                        )
                        continue
