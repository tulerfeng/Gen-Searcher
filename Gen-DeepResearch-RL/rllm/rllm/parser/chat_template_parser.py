...
class QwenChatTemplateParser(ChatTemplateParser):
    ...
    def process_image_data(self, messages):
        ...
        image_data = []
        raw_max = (os.environ.get("QWEN_VL_MAX_PIXELS") or "160000").strip()
        try:
            max_pixels = int(raw_max) if raw_max else 160000
        except ValueError:
            max_pixels = 160000
        ...
        for message in messages:
            if "images" in message and message["images"] is not None:
                ...
                if not images or images[0] is None:
                    continue
                for image in images:
                    image_dict = dict(image) if isinstance(image, dict) else {"image": image}
