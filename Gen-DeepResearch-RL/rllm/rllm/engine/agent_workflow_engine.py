...
class AgentWorkflowEngine:
    ...
    async def process_task_with_retry(
        ...
        try:
            for retry_attempt in range(1, self.retry_limit + 1):
                ...
                # 传递 global_step 给 workflow
                workflow_kwargs = {**kwargs, "global_step": self.current_step}
                episode = await workflow.run_with_termination_handling(
                    task=task, uid=uid, **workflow_kwargs
                )

                # Display rewards for all trajectories（保留两位小数）
                rewards_str = ", ".join(
                    [f"{traj.name}: {traj.reward:.2f}" for traj in episode.trajectories]
                )
                extra_reward_str = ""
                try:
                    text_coef = float(os.environ.get("GEN_REWARD_TEXT_COEF", "0").strip() or "0")
                except Exception:
                    text_coef = 0.0
                if text_coef > 0:
                    ep_info = episode.info or {}
                    img_r = ep_info.get("image_reward")
                    txt_r = ep_info.get("text_reward")
                    weighted_r = ep_info.get("reward_weighted")
                    parts = []
                    if isinstance(img_r, (int, float)):
                        parts.append(f"image_reward: {float(img_r):.2f}")
                    if isinstance(txt_r, (int, float)):
                        parts.append(f"text_reward: {float(txt_r):.2f}")
                    if isinstance(weighted_r, (int, float)):
                        parts.append(f"weighted: {float(weighted_r):.2f}")
                    if parts:
                        extra_reward_str = ", " + ", ".join(parts)
                colorful_print(
                    f"[{uid}] Rollout completed. Rewards: {rewards_str}{extra_reward_str}, Termination: {episode.termination_reason}",
    ...
    def transform_results_for_verl(
        ...
        for i, episode in enumerate(episodes):
            ...
            # Gen Image: 文本 reward 启用时，供 wandb critic 记录 image_reward / text_reward
            ep_info = episode.info or {}
            _img_r = ep_info.get("image_reward")
            _txt_r = ep_info.get("text_reward")
            image_rewards_list.extend([_img_r] * total_steps)
            text_rewards_list.extend([_txt_r] * total_steps)
        ...
        if cf.enable:
            for i in range(len(episode_ids)):
                ...
                if (
                    ...
                    or (
                        getattr(cf, "mask_repeated_response", True)
                        and termination_reason
        ...
        # 若某样本下所有 rollout 的 reward 均为 0，则整组不参与梯度（mask_all_failed_episodes）
        if cf.enable and getattr(cf, "mask_all_failed_episodes", True):
            from collections import defaultdict
            episode_to_indices = defaultdict(list)
            for i, eid in enumerate(episode_ids):
                episode_to_indices[eid].append(i)
            for _eid, indices in episode_to_indices.items():
                rewards_for_episode = [traj_rewards[i] for i in indices]
                if rewards_for_episode and max(rewards_for_episode) <= 0:
                    for i in indices:
                        is_valid[i] = False
        ...
        # Gen Image: text reward 启用时含 image_reward / text_reward（None 的用 nan 便于后续 mean）
        if image_rewards_list:
            arr = np.array([float(x) if x is not None else np.nan for x in image_rewards_list], dtype=np.float64)
            non_tensors["image_reward"] = arr
        if text_rewards_list:
            arr = np.array([float(x) if x is not None else np.nan for x in text_rewards_list], dtype=np.float64)
            non_tensors["text_reward"] = arr
