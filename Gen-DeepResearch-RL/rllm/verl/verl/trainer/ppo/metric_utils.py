...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    # Gen Image: text reward 启用时，单独记录 image_reward / text_reward 均值（wandb critic）
    if "image_reward" in batch.non_tensor_batch:
        arr = np.asarray(batch.non_tensor_batch["image_reward"], dtype=np.float64)
        valid = ~np.isnan(arr)
        if np.any(valid):
            metrics["critic/reward_image/mean"] = float(np.nanmean(arr))
    if "text_reward" in batch.non_tensor_batch:
        arr = np.asarray(batch.non_tensor_batch["text_reward"], dtype=np.float64)
        valid = ~np.isnan(arr)
        if np.any(valid):
            metrics["critic/reward_text/mean"] = float(np.nanmean(arr))
