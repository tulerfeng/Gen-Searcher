...
class SGLangHttpServer:
    ...
    def __init__(
        ...
        # 强制 70k，避免多轮 agent prompt 超长时 headroom 为负（原 prompt_length+response_length 仅 ~8k）
        self.config.max_model_len = 70000
    ...
    async def generate(
        ...
        # 单轮生成长度固定为 response_length（即 sglang_response_length = GEN_MAX_NEW_TOKENS_PER_TURN + 256）
        # 仅在剩余上下文不足时截断，且保证至少为 1，避免出现负值导致 SGLang 报错
        headroom = self.config.max_model_len - len(prompt_ids) - 1
        max_new_tokens = max(1, min(self.config.response_length, headroom))
...
class SGLangReplica(RolloutReplica):
    ...
    async def launch_servers(self):
        ...
        for node_rank in range(self.nnodes):
            ...
            sglang_env = {"RAY_EXPERIMENTAL_NOSET_CUDA_VISIBLE_DEVICES": "1"}
            if os.environ.get("CUDA_LAUNCH_BLOCKING"):
                sglang_env["CUDA_LAUNCH_BLOCKING"] = os.environ.get("CUDA_LAUNCH_BLOCKING")
            server = SGLangHttpServer.options(
                scheduling_strategy=ray.util.scheduling_strategies.NodeAffinitySchedulingStrategy(
                    node_id=node_id,
                    soft=False,
                ),
                runtime_env={"env_vars": sglang_env},
                name=name,
            ).remote(
                config=self.config,
                model_config=self.model_config,
